...
class Evaluator
  ...
  def Evaluator.evaluate(times, sides)
  ...
  def initialize
    @diceParser = Parser.new("dice roller") do
      ...
      rule :assign do
        match('(', 'set', :var, :expr, ')') {|_,_,a,b,_| @components["#{a}"] = b}
      end
      ...
      rule :expr do
        ...
        # om man inte behöver göra testfall kan koden innanför måsvingarna för "not" bytas ut mot !a
        match(:term)
      end
