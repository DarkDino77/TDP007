...
require './constraint_networksX3000'
...
class ConstraintParser < Parser
  ...
  def initialize
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        #   conn_a,conn_b,conn_c=get_connectors(a,'-',b)
        #   # a-b=c <=> a=b+c
        #   adder= Adder.new(conn_b,conn_c,conn_a)
        #   p "Adder värde ", adder.out.value, conn_a.value, conn_b.value, conn_c.value
        #   adder
        # end
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          adder= Sub.new(conn_a, conn_b,conn_c)
          p "Adder värde ", adder.out.value, conn_a.value, conn_b.value, conn_c.value
          adder
        end
      ...
      rule :term do
        ...
        match(:term, '*', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'*',b)
          adder= Multiplier.new(conn_a,conn_b,conn_c)
          p "Multiplier värde ", adder.out.value
          adder
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    p conn_a, conn_b
    # if conn_a.is_a?(ArithmeticConstraint)
    #   if conn_a.b.name =~ /-/ or conn_a.b.name =~ /\// 
    #     conn_a = conn_a.out
    #   else
    #     conn_a = conn_a.b
    #   end
    # end
    # if conn_b.is_a?(ArithmeticConstraint)
    #   if conn_b.b.name =~ /-/ or conn_b.b.name =~ /\// 
    #     conn_b = conn_b.out 
    #   else
    #     conn_b = conn_b.b
    #   end
    ...
    name_c="Connerctors #{conn_a.name}#{op}#{conn_b.name}"
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(Connector) then
      exp,conn=rh,lh_conn
      @connectors.delete(rh_conn.name)
    else 
      if lh.is_a?(Connector) then
        exp,conn=lh,rh_conn
    ...
    #p "conns ", exp.b.name, conn
    # Tror denna tar fel
    # temp_val = 0
    # if exp.b.name =~ /\d+-\d+/ or exp.b.name =~ /\d+\/\d+/ 
    #   temp_val = exp.b.value 
    # else
    #   temp_val = exp.out.value 
    # end
  
    temp_val = exp.out.value 
    exp.out=conn
    conn.add_constraint(exp)
    conn.user_assign(temp_val) unless temp_val.is_a?(FalseClass)
    @connectors[conn.name] = conn

  end

  def get_connector(expr)
    if expr.is_a?(Connector)
      return expr
    elsif expr.is_a?(ArithmeticConstraint)
      return expr.out
    end
    # raise ParseError "expression was not parsed as a constraint or connector"
  end
