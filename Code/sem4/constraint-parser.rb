...
class ConstraintParser < Parser
  ...
  def replace_conn(lh,rh)
    ...
    # Detta är för att om båda sidorna är connctors att 
    # sätta ihop genom en adder med plus 0 f = 7 blir till f = 7 + 0
    if rh.is_a?(Connector) and lh.is_a?(Connector)  then
      exp = Adder.new(rh,ConstantConnector.new("zero",0),lh)
      conn = lh_conn
    end
