...
class ConstraintParser < Parser
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    conn_b = conn_b.b if op == "-" or op == "/"
    p "connectors in get_connectors: ", conn_a, op , conn_b
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(ArithmeticConstraint) then
      ...
      # else
      # end 
    # p "replace_conn debug: ", lh, rh, lh_conn, rh_conn, exp.out.value, conn
  ...
  def get_connector(expr)
    # p "expr in get_connector: ",expr.class
    if expr.class == Connector
      # p "expr är en conn"
      return expr
